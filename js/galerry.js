...
const createImage = () => {
    ...
    images.forEach((img) => {
        ...
        imageArray.push(createLi);
    });

    return gallery.append(...imageArray);
};

createImage();
const clickOnImage = (event) => {
    if (event.target.nodeNane !== "IMG") {
        return;
    }

    const instance = basicLightbox.create(`<img src="${event.target.datsset.source}" width="800" height="600">`);
    instance.show();
};
